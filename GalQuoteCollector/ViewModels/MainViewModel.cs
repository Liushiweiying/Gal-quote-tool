...
public partial class MainViewModel : ObservableObject
{
    ...
    private void ImportItems(List<ImportItem> items)
    {
        ...
        {
            if (IsDuplicateQuote(item.Text, item.GameName)) { skipped++; continue; }
        ...
        StatusText = skipped > 0
            ? $"已导入 {imported} 条语录（跳过重复 {skipped} 条）"
            : $"已导入 {imported} 条语录";
    }

    /// <summary>Duplicate = same text AND same game name (case-insensitive).</summary>
    private bool IsDuplicateQuote(string text, string gameName)
    {
        var t = (text ?? "").Trim();
        var g = (gameName ?? "").Trim();
        return _allQuotes.Any(q =>
            string.Equals(q.Text.Trim(), t, StringComparison.OrdinalIgnoreCase) &&
            string.Equals(q.GameName.Trim(), g, StringComparison.OrdinalIgnoreCase));
    }
    ...
    [RelayCommand]
    private void CopyQuoteMarkdown()
    {
        if (SelectedQuote == null) return;
        var q = SelectedQuote;
        var sb = new System.Text.StringBuilder();
        if (!string.IsNullOrWhiteSpace(q.GameName))
            sb.AppendLine($"- {q.GameName}");
    ...
    private async Task CheckForUpdates()
    {
        try
        {
            ...
            {
                StatusText = $"已跳过版本 {info.Tag}";
                return;
            }
            StatusText = $"发现新版本 {info.Tag}";
            ShowUpdateDialog(info);
        }
        catch
        {
            StatusText = "检查更新失败（网络不可用？）";
        }
    }

    private bool IsUpdateSkipped(string tag)
        => string.Equals(_settingsService.LoadHotkeyConfig().SkippedUpdateVersion, tag, StringComparison.OrdinalIgnoreCase);

    private void ShowUpdateDialog(UpdateService.UpdateInfo info)
    {
        var dlg = new Views.UpdateDialog(info, AppVersion, _updateService, onSkipped: tag =>
        {
            var cfg = _settingsService.LoadHotkeyConfig();
            cfg.SkippedUpdateVersion = tag;
            _settingsService.SaveHotkeyConfig(cfg);
        })
        {
            Owner = _window
