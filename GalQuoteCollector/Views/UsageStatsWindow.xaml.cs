...
public partial class UsageStatsWindow : Window
{
    ...
    private void OnDayMode(object s, RoutedEventArgs e)
    {
        _weekMode = false;
        DayModeBtn.IsChecked = true;
        WeekModeBtn.IsChecked = false;
        DateNavPanel.Visibility = Visibility.Visible;
        RefreshView();
    }

    private void OnWeekMode(object s, RoutedEventArgs e)
    {
        _weekMode = true;
        DayModeBtn.IsChecked = false;
        WeekModeBtn.IsChecked = true;
        DateNavPanel.Visibility = Visibility.Collapsed;
        RefreshView();
    }
    ...
    private void RefreshWeek()
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            var day = _data.GetDay(_currentDate.AddDays(-i).ToString("yyyy-MM-dd"));
            if (day == null) continue;
            foreach (var kv in day)
            {
                if (kv.Key == ToolKey) { toolTotal += kv.Value.Seconds; continue; }
                if (apps.TryGetValue(kv.Key, out var cur))
                    apps[kv.Key] = (cur.secs + kv.Value.Seconds, kv.Value.Name);
                else
                    apps[kv.Key] = (kv.Value.Seconds, kv.Value.Name);
            }
        }
        ...
        int rank = 0;
        foreach (var kv in apps.OrderByDescending(x => x.Value.secs))
        {
            rank++;
        ...
        var totalCard = CreateCard();
        AddToCard(totalCard, CreateLabelRow("应用合计", FormatTime(apps.Sum(x => x.Value.secs)), GrayText));
        StatsPanel.Children.Add(totalCard);
    }
